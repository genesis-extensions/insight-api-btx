...
StatusController.prototype.circulation = function(req, res) {
  var self = this;
  var subsidy;
  if (self.node.services.bitcoind.height % (1440 * 7 * 4 * 12) == 0) { // TODO modulo
    subsidy = 1000 * 1440 * 7 * 4 * 12 / 100
  } else if (self.node.services.bitcoind.height % (1440 * 7 * 4) == 0) {
    subsidy = 1000 * 1440 * 7 * 4 / 100
  } else if (self.node.services.bitcoind.height % (1440 * 7) == 0) {
    subsidy = 1000 * 1440 * 7 / 100
  } else if (self.node.services.bitcoind.height % (1440) == 0) {
    subsidy = 1000 * 1440 / 100
  } else {
    subsidy = 1000 / 100 // * 7 super , *7*4 mega, *7*4*12 ultra. //todo define block with those
  }
  var halvings = Math.floor((self.node.services.bitcoind.height) / 210000);
  ...
  var coins = ((self.node.services.bitcoind.height) * subsidy)
